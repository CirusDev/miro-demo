...
const SELECTION_NET_TRESHOLD = 5
...
export const Canvas = ({ boardId }:Props) => {
  ...
  const startMultiSelection = useCallback((current: Point, origin: Point) => {
    if (
      Math.abs(current.x - origin.x) + Math.abs(current.y - origin.y) > SELECTION_NET_TRESHOLD
    ) {
      setCanvasState({
        mode: CanvasMode.SelectionNet,
        origin,
  ...
  return (
    <main
      className='relative h-full w-full bg-neutral-100 touch-none'
    >
